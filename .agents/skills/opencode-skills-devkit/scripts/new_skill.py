...
def skill_paths(root: Path, location: str, target: str, name: str) -> list[Path]:
    ...
    if target not in {"agents", "opencode", "claude", "both"}:
        raise ValueError("target must be agents, opencode, claude, or both")

    bases: list[Path] = []
    if location == "project":
        if target == "agents":
            bases.append(root / ".agents" / "skills")
        ...
        if target == "agents":
            bases.append(Path("~/.agents/skills").expanduser())
        if target in {"opencode", "both"}:
...
def main() -> int:
    ...
    parser.add_argument(
        "--compatibility",
        default=None,
        help="Optional compatibility label",
    )
    parser.add_argument(
        "--metadata",
        action="append",
    ...
    parser.add_argument(
        "--target",
        choices=["agents", "opencode", "claude", "both"],
        default="agents",
        help="Where to create the skill (default: agents)",
    )
    parser.add_argument(
        "--location",
