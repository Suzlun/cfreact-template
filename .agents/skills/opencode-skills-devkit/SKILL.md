...
Project-local (shared Agent Skills location, with English templates by default):

```bash
python3 .agents/skills/opencode-skills-devkit/scripts/new_skill.py \
  --name my-skill \
...
The default `--target agents` writes one shared bundle to `.agents/skills/`
(or `~/.agents/skills/` with `--location global`). Explicit `--target opencode`,
`--target claude`, and `--target both` remain available for runtime-specific installs.

3. Validate skills

```bash
python3 .agents/skills/opencode-skills-devkit/scripts/validate_skills.py --root .
```

To also validate global skill locations:

```bash
python3 .agents/skills/opencode-skills-devkit/scripts/validate_skills.py --root . --include-global
```
