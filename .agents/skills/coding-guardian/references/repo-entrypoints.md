...
- `openspec/config.yaml`: OpenCode ownership of integrated React prototypes per public app and app-specific Storybook story references, Request semantics, and approved-design handoff rules
- `mockups/AGENTS.md`: integrated React mocks, Storybook args and story references, shared compiler configuration, and desktop/mobile verification
- `docs/storybook-host.md`: standard Dev Container, native and shared hosting, runtime project configuration, and lifecycle commands
...
- `.agents/skills/openspec/openspec-review/SKILL.md`: repository-specific semantic review of OpenCode planning artifacts
...
- `scripts/openspec/verify-change-proposal.mjs`: proposal structure and nonempty `UX-Mode` evidence fields; semantic review reads Request's `UI Mock References`, actual React source and Storybook story references, and proposal's approved `Design Source` scope for `SHAPE`, or existing production `Continuity Source` for `CONTINUITY`
