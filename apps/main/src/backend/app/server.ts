...
import healthApi from '@cfreact-template/main/backend/generated/api/health/health';
import helloApi from '@cfreact-template/main/backend/generated/api/hello/hello';
import type { components } from '@cfreact-template/main/backend/generated/api/openapi';
import usersApi from '@cfreact-template/main/backend/generated/api/users/users';
import { inspectSafeErrorResponse } from '@cfreact-template/main/backend/platform/http/responseValidation';
import { logFailure } from '@cfreact-template/main/backend/platform/observability/logger';
...
// 外部所有HTTPサブツリーは、製品ミドルウェアより前に実在する統合ごとに明示的にマウントする。

// TypeSpec所有経路だけに製品CORSを適用し、外部所有経路のヘッダー契約を変更しない。
const productCors = cors({
  origin: ['http://localhost:5173', 'http://127.0.0.1:5173'],
  credentials: true,
});
app.use('/api/*', productCors);
app.use('/health', productCors);

// 利用者APIの基底パスだけでcore SDKを構築し、挨拶と稼働確認を共有業務から独立させる。
app.use('/api/v1/users/*', async (c, next) => {
  // `Hono` の単一ワイルドカードは基底パスにも一致するため、重複するミドルウェア登録なしで一度だけ構築する。
  const services = createServices(c.env);
  c.set('coreSdk', services.coreSdk);
  await next();
});

// `Orval` の標準検証処理が返す不正入力応答だけを、`TypeSpec` の安全なエラー契約へ統一する。
const normalizeProductBadRequest: MiddlewareHandler = async (c, next) => {
  await next();
  if (c.res.status === 400) {
...
app.use('/api/*', normalizeProductBadRequest);
app.use('/health', normalizeProductBadRequest);

// `TypeSpec` から生成したリソース別 `Hono` アプリを合成し、製品APIのパス、HTTPメソッド、スキーマを手書きしない。
app.route('/', usersApi);
app.route('/', helloApi);
app.route('/', healthApi);
