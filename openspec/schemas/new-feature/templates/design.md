...
- <!-- TODO: repository に含める migration / rollback 設計と、local または CI で確認する方法を日本語で記述する。N/A の場合は理由を日本語で書く。 -->

## Merge Verification

- <!-- TODO: merge 前に実行する repository-local または CI の検証 command、review gate、客観的な完了条件を日本語で記述する。release 実行、外部環境の操作、credential、外部承認は記述しない。 -->
