...
## 5. import / export（公開境界の作法）

### 5.1 エイリアスを使う（相対パスでレイヤーを跨がない）
...
### 5.2 `index.ts` は公開境界（re-export 専用）
...
### 5.5 ESLint 無効化コメント（eslint-disable）の運用
...
### 9.1 `server-domain` / `server-usecases`（純粋性）
