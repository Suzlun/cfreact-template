...
提案者はYAGNIを徹底し、依頼または確認済みの解釈から顧客価値が明らかな成果だけを仕様候補にします。規格準拠、RFC準拠、パッケージ、実装方式は、顧客がその外部から観測できる効果自体を求めている場合を除いて手段として扱います。

### 永続的な振る舞い契約

主仕様は `openspec/specs/<capability>/spec.md` に置きます。活動中の Change は `openspec/changes/<change-id>/specs/<capability>/spec.md` に差分を持ちます。

Requirement と Scenario は、利用者または外部契約から観測でき、顧客価値へ直結する終端状態だけを表します。特定の規格、RFC、ファイル、パッケージ、関数、補助処理、移行作業、試験階層を製品要件として記載しません。ただし、利用者が求める体験そのものを表す可視のUI構成または配置は成果の制約として記載できます。内部コンポーネントの構造は手段です。

### 計画完了と実装時判断

`Change` は、確認済みの顧客成果、外部契約の意味、およびアーキテクチャ、セキュリティ、データ、依存関係、実行環境、対象範囲、重要な UX 方針の物質的境界が解決済みであれば実装へ進めます。`TypeSpec` または実装が具体表現を選ぶ必要があるだけでは計画の欠落としません。ただし、正確な表現自体が確認済みの成果制約または外部契約である場合は、実装前に解決します。
