...
3. Open the HTML preview in an isolated browser session: `agent-browser open file://{absolute_path}.wireframe.html --session openspec-designer-{change-id} --allowed-domains localhost,127.0.0.1`
...
**Canonical schema:** This JSON schema is the canonical layout format used across the pipeline. It describes only the visible layout. It is not a requirements-coverage, implementation, or internal-state artifact.
...
- **Phase 4 (Grid):** Two major zones — sidebar (240px, fixed, persistent navigation) + content area (`grow: true`). Sidebar exists only because persistent navigation is necessary for the monitoring task.
