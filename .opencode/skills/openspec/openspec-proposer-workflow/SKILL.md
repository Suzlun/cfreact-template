...
- Goal: complete intent, proposal, Specs, design, and tasks along the artifact graph and make `pnpm exec openspec validate --type change <id> --strict --no-interactive` pass.
...
- If the target does not exist, create it with `pnpm exec openspec new change "<change-id>"`.

### 2. Understand current state

- Read `AGENTS.md`, `openspec/config.yaml`, and the active schema instructions.
- Check status with `pnpm exec openspec status --change "<change-id>" --json`.
...
- Get instructions with `pnpm exec openspec instructions intent --change "<change-id>" --json`.
...
- Include applicable codegen, lint, test, build, and review steps, with dependencies and safe parallel groups explicit.
...
- Confirm strict validation passes and readiness is `READY`.
- Report the confirmed intent path and owner-approved summary.
- Report changed artifacts, commands run, accepted specialist decisions, and remaining risks or decisions.
