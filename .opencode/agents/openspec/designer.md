...
- When inspecting the running application, use only `http://localhost:5173` or `http://127.0.0.1:5173`. Open it as `agent-browser open <local-url> --session openspec-designer-<change-id> --allowed-domains localhost,127.0.0.1`, then append the same `--session openspec-designer-<change-id>` after every related browser action. Never reuse the default session.
...
- Require one or two additional official sources when they can test the interaction or information-architecture choice, such as W3C ARIA Authoring Practices, IBM Carbon Patterns, Microsoft Fluent 2, or Base UI accessibility documentation.
...
5. Existing `packages/ui/styles/globals.css` tokens, shadcn/ui and Base UI component boundaries, Tailwind conventions, and repository ownership rules.
...
- External references are advisory comparison evidence only. Confirmed intent and proposal own product scope; implementation and active wireframes own continuity; the existing shared UI tokens and component language own visual decisions.
...
- Apply the reduction rules plus the `impeccable` and `design-audit` quality gates after synthesis. Repository constraints override every external reference.
...
5. Compare the reported patterns against confirmed intent, continuity, existing shared UI tokens and components, reduction rules, and accessibility constraints. Record which patterns inform the proposal, why they fit, and what must not transfer.
...
7. Generate the matching preview with `node .opencode/skills/wireframe/scripts/generate-preview.mjs <json-path>`.
8. Open only the generated local HTML preview with `agent-browser`, using the same `--session openspec-designer-<change-id>` and `--allowed-domains localhost,127.0.0.1` boundary used for current-surface inspection. Record every design correction against the JSON source, regenerate the preview, and inspect it again; never edit generated HTML or navigate to an external URL.
9. Apply the reduction rules and the `impeccable` and `design-audit` quality gates again after rendering without removing context or navigation required for continuity. If the JSON changes, regenerate and inspect the preview again.
...
- Never navigate `agent-browser` to an external URL. Use it only for generated local wireframe previews and rendering evidence.
