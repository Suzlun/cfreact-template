...
- OpenDesign works in this repository and owns `PRODUCT.md`, the single root `mockups/` prototype, and planning artifacts. Canonical React TypeScript source lives in `mockups/src/**`, centered on `App.tsx` and `main.tsx`. `PRODUCT.md` provides product context, not product requirement authority; `request.md` remains confirmed request evidence.
- OpenDesign updates prototype source and owner-confirmed Request together. Its agent runs `pnpm build:mockup` in the repository, then opens `mockups/index.html` as a normal static artifact in the built-in Prototype Preview. The agent's build environment supplies Node.js and repository dependencies; preview consumes the built files.
- One prototype serves multiple Changes. Request's `UI Mock References` points to relevant routes and scenarios such as `mockups/index.html?scenario=default#/` or `mockups/index.html?scenario=users-error#/users`; proposal's `Design Source` records the same references, adopted scope, and owner confirmation. Shared viewpoint changes require re-evaluating every affected Request. Root `mockups/` remains in place when Changes are archived. See `mockups/AGENTS.md` for source, build, and preview rules.
...
- Prototype: `pnpm build:mockup`, then open `mockups/index.html` in OpenDesign's built-in Prototype Preview.
...
- `mockups/src/**` uses React, fixtures, local state, and direct shared implementations through public `@cfreact-template/ui` subpaths, without actual API or database access. Vite builds it with the existing shared UI Tailwind CSS 4 and React Compiler configuration into one IIFE `mockups/dist/prototype.js` and `mockups/dist/prototype.css`. Stable `mockups/index.html` references `./dist/prototype.js` and `./dist/prototype.css`. Keep `dist` version-controlled, rebuild it with source changes, and never edit it manually. Production imports neither prototype source nor output; approved presentation is formalized into app/UI source during implementation.
