...
- OpenDesign works in this repository and owns app prototypes under root `mockups/` and planning artifacts. Each publicly exposed `apps/<app>` pairs with one integrated prototype in `mockups/<app>`. Canonical React TypeScript source lives in `mockups/<app>/src/**`, centered on `App.tsx` and `main.tsx`; the current example is `mockups/main/src/App.tsx`. `request.md` remains confirmed request evidence.
