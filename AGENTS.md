...
- Candidate ideas MUST remain confined to deliberation. Non-goals, rejected alternatives, and concepts judged unnecessary or wrong MUST NOT survive that deliberation. After a concept is rejected, its absence, prohibition, non-adoption, or wrongness MUST NOT be preserved as a requirement, constraint, rationale, warning, or any other repository artifact. Every reference to that concept MUST be removed from the current repository. The repository MUST retain only confirmed customer outcomes, external contracts, and the adopted content required to realize them.
- Artifacts MUST be evaluated by the necessity of what they contain, not by counting what they omit. Brevity and omission MUST receive positive weight. Before alleging an omission, the reviewer MUST prove the exact confirmed customer outcome or external contract that the omitted consideration is indispensable to protect. Every recorded consideration MUST have a proven purpose and causal contribution to customer value. A consideration that cannot meet that burden MUST be treated as taking attention, judgment, and implementation resources away from customer value and MUST be removed.
...
- Shared project skills live in `.agents/skills/`, discovered through OpenCode's project Agent Skills compatibility. OpenDesign's OpenCode runtime uses the same repository root and shared skills. Agent definitions remain in `.opencode/agents/`; command definitions remain in `.opencode/commands/`.
...
- OpenDesign is the canonical entry for product shaping and owns Request dialogue and planning artifacts. In `SHAPE`, Request and Mock evolve together, with owner-confirmed content saved as it becomes clear.
- OpenDesign works in this repository and owns `PRODUCT.md`, app prototypes under root `mockups/`, and planning artifacts. Each publicly exposed `apps/<app>` pairs with one integrated prototype in `mockups/<app>`. Canonical React TypeScript source lives in `mockups/<app>/src/**`, centered on `App.tsx` and `main.tsx`; the current example is `mockups/main/src/App.tsx`. `PRODUCT.md` provides whole-product context, including each app's distinct purpose, not product requirement authority; `request.md` remains confirmed request evidence.
- OpenDesign updates prototype source and owner-confirmed Request together. Its agent runs `pnpm build:mockup` for all existing app prototypes or `pnpm build:mockup main` for only `main`, then opens the applicable `mockups/<app>/index.html` as a normal static artifact in the built-in Prototype Preview. The agent's build environment supplies Node.js and repository dependencies; preview consumes the built files.
- Each app's N routes/scenarios may serve M Changes; one Change may reference multiple app prototypes when its confirmed outcomes require them. Request's `UI Mock References` identifies the app through exact paths such as `mockups/main/index.html?scenario=default#/` or `mockups/main/index.html?scenario=users-error#/users`; proposal's `Design Source` records the same references, adopted scope, and owner confirmation. Shared UI or viewpoint decisions require re-evaluating every affected app and Request. Root `mockups/` and its app prototypes remain in place when Changes are archived. See `mockups/AGENTS.md` for source, build, and preview rules.
- `request.md` is owner-controlled request evidence. `Request-Status: CONFIRMED` means all currently saved content is confirmed, not that discovery is complete. An explicit, unambiguous owner statement is sufficient confirmation evidence for an immediate update by OpenDesign; ask again only when its meaning or binding force is unclear.
- Repository evidence, common practice, security recommendations, implementation necessity, downstream artifacts, and tests cannot create product Requirements. OpenCode implementation returns `OPENDESIGN_PLANNING_REQUIRED` when product decisions are missing or planning artifacts contradict one another.
...
- Production stores the same high-entropy `CORE_API_TOKEN` as a secret on main and core. Deployment uploads it from the GitHub `production` Environment through an ephemeral secrets file and never places it in Wrangler vars or Terraform state.
...
- `mockups/<app>/src/**` uses React, fixtures, local state, and direct shared implementations through public `@cfreact-template/ui` subpaths, without actual API or database access. Shared root `mockups/vite.config.ts`, `mockups/tsconfig.json`, and `mockups/AGENTS.md` serve the app prototypes. Vite uses the existing shared UI Tailwind CSS 4 and React Compiler configuration to build one IIFE `mockups/<app>/dist/prototype.js` and `mockups/<app>/dist/prototype.css` per app. Each stable `mockups/<app>/index.html` references `./dist/prototype.js` and `./dist/prototype.css`. Keep each app's source and rebuilt, version-controlled `dist` coherent in the same commit; never edit output manually. Production imports neither prototype source nor output; approved presentation is formalized into the scoped `apps/<app>` and shared UI during implementation.
...
- React Compiler is mandatory for handwritten frontend/domain/UI and mockup code and is configured only through `@cfreact-template/build-config/react-compiler`; runtime source must never import build tooling.
...
- App server dependency direction is either `entry -> app -> generated Resource routes -> Handler -> @cfreact-template/core-sdk` for a direct domain-operation mapping or `entry -> app -> generated Resource routes -> Handler -> Service -> @cfreact-template/core-sdk / declared external client` for an app-specific use case. App backends never import core implementation, Repository, schema, or D1 bindings.
- Core server dependency direction is `entry -> app -> generated Resource routes -> Handler -> Service -> Repository -> schema/Platform`. The core Service owns domain operations, invariants, transitions, and effect coordination. Core never imports its own SDK. Module-internal relative imports are allowed, but cross-Module relatives and parent escapes are not, except for generator-owned smart-handler preambles that reach the same Resource's generated files.
- Each `apps/*` owns its use cases. A use case is identified by its intended user, situation, purpose, and outcome; different intended users mean different use cases even when they invoke the same domain operations in the same order. When all four are the same across apps, confirm whether separate apps are needed, but preserve the split when it is an owner-confirmed request. Duplication alone never moves an app use case into core.
- The core API is the domain boundary consumed by app use cases. It exposes domain concepts, state, operations, queries, invariants, transitions, and consistency in domain language; it owns no app-specific user journey or workflow. Avoiding remote Repository methods and persistence-shaped DTOs is a consequence of this boundary, not its definition.
- Main-specific decisions and compositions of core operations or external services belong in the app Resource's Service. Main Services may depend on `@cfreact-template/core-sdk`; core Services may not. Keep direct Handler-to-core-SDK mapping when no app-specific decision or composition exists instead of adding a pass-through Service.
- App Services own cross-domain and external-service sequencing. If multiple changes must be atomic to preserve a core-domain invariant, expose one core domain operation instead of recreating the transaction in an app. Core never depends on an app-specific workflow, and apps never reconstruct core transactions.
- Current public Resources are `users`, `hello`, and `health`. Public `users` directly maps core domain operations and queries to the public app contract because it has no main-specific composition. `packages/core` owns the users domain Service, Repository, Drizzle schema, D1 binding, email effect, and migration stream. `hello` and `health` remain in `apps/main`.
- Backend external-package imports are denied by default. The allowlist is limited to Hono in app/Handlers/HTTP Platform, generated Hono validation dependencies, ULID in Services, Drizzle in Repositories/schemas/database Platform, `cloudflare:email` in email Platform, Workers types in shared Types, and Vitest in pure same-Resource or core SDK transport test files. Handler and Service code must not bypass declared dependencies through HTTP globals; Handlers must not read `env` directly.
...
- The core HTTP contract requires Bearer authentication and is independent of its hosting provider. The core SDK receives a base URL, token, and standard `fetch` at the app composition root, rejects non-loopback plaintext HTTP and redirects, and overwrites caller-supplied authorization. Current Cloudflare deployment injects the `CORE_API` Service Binding transport and keeps core without public or preview routes; external runtimes use certificate-validated HTTPS. Main maps internal authentication, transport, parse, and unknown-status failures to its logged fixed 500 response without retries or fallback data access.
...
- `NONE` has no user-visible surface change and needs no Mock. `CONTINUITY` preserves identified existing production evidence. `SHAPE` uses OpenDesign and requires a confirmed Request and owner-accepted Mock before proposal convergence.
- For `SHAPE`, proposal convergence requires mutual completeness, rationale, and consistency between Request and Mock. `proposal.md` records `Design Source` with an OpenCode-readable path or stable identifier, the related screens, flows, and states, and brief adoption evidence.
- Actual UI changes require production-designer involvement and real-browser review on desktop and mobile. Implement the accepted composition, hierarchy, actions, navigation, copy, states, responsive behavior, and visual direction faithfully. Complete only necessary production states deducible from the confirmed contract and accepted design.
- OpenCode and the user-selected `openspec/applier` implement only Planning Ready Changes. Their planning-file edits are limited to `tasks.md` progress. UI implementation follows `PRODUCTION_UI -> WIRING -> POLISH -> REVIEW`; unresolved product decisions return to OpenDesign.
...
- OpenSpec Specs are the authority for observable behavior; the confirmed Request is their semantic basis. OpenSpec is not a detailed implementation plan.
- OpenSpec is pinned to `1.11.0`. Create Changes through the CLI with `--schema behavior-change` for `BEHAVIOR` or `--schema architecture-change` for `ARCHITECTURE`. Never hand-create a directory under `openspec/changes/`.
- `pnpm gen:openspec` uses OpenSpec `1.11.0` with the custom profile for `new`, `continue`, `update`, `apply`, `verify`, `sync`, and `archive`. The official `--tools agents` integration generates shared skills at `.agents/skills/openspec-*/SKILL.md`; `--tools opencode` generates only commands at `.opencode/commands/opsx-*.md`. Generated files must not be hand-edited. Handwritten repository-specific supplemental skills retain their nested structure under `.agents/skills/openspec/**`.
- OpenDesign owns `request.md`, `proposal.md`, Specs, `design.md`, and the scope of `tasks.md`. The user selects `openspec/applier` as a primary agent for implementation, never through subagent delegation. Generated skills provide generic traversal within these ownership boundaries.
- Both Change schemas begin with `Request-Status: CONFIRMED` in `request.md`. OpenDesign confirms the affected user, current situation, motivation, expected value, and desired outcome as needed. Confirmed Background and Motivation explain the Request but do not create product Requirements by themselves.
- During planning, OpenDesign asks one focused question for each material semantic choice not entailed by confirmed content. Clear owner answers are immediately recorded with their confirmation evidence.
...
- Planning Ready requires resolved product and material design decisions, consistent planning artifacts, and the applicable UX evidence. For greenfield work, OpenDesign shapes an integrated journey and Mock, then groups work into outcome-bound Changes. Re-evaluate affected Changes when shared decisions change; independently ready waves may be handed off without waiting for the whole product. `IDEA -> CHANGE_SCAFFOLDED -> SHAPING -> PLANNING_READY -> IMPLEMENTING -> VERIFIED -> ARCHIVED` describes conceptual lifecycle stages using existing artifacts and progress.
