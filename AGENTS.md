...
- OpenCode is the canonical entry for product shaping and owns Request dialogue and planning artifacts. In `SHAPE`, Request and Mock evolve together, with owner-confirmed content saved as it becomes clear.
- During planning, the OpenCode primary agent owns Request dialogue, planning artifacts, and integrated React mocks in `mockups/<app>/src/**`. Each public `apps/<app>` has one integrated mock. `App.stories.tsx` defines initial screens and states through Storybook args; the React mock owns the interactive journey. `request.md` remains confirmed request evidence.
- OpenCode updates the mock and owner-confirmed Request together, then verifies the running Storybook on desktop and mobile. Dev Container is the standard runtime. `pnpm storybook:host` serves the project selector and development previews; `pnpm storybook:mockup` starts the project's mock Storybook directly. `pnpm build:mockup` verifies the static Storybook build.
- Each app's N routes/scenarios may serve M Changes; one Change may reference multiple app prototypes when its confirmed outcomes require them. Request's `UI Mock References` identifies the app through exact paths such as `mockups/main/src/App.stories.tsx#Home` or `mockups/main/src/App.stories.tsx#UsersError`; proposal's `Design Source` records the same references, adopted scope, and owner confirmation. Shared UI or viewpoint decisions require re-evaluating every affected app and Request. Root `mockups/` and its app prototypes remain in place when Changes are archived. See `mockups/AGENTS.md` for source, build, and preview rules.
- `request.md` is owner-controlled request evidence. `Request-Status: CONFIRMED` means all currently saved content is confirmed, not that discovery is complete. An explicit, unambiguous owner statement is sufficient confirmation evidence for an immediate update by OpenCode; ask again only when its meaning or binding force is unclear.
- Repository evidence, common practice, security recommendations, implementation necessity, downstream artifacts, and tests cannot create product Requirements. OpenCode implementation returns `PLANNING_REQUIRED` when product decisions are missing or planning artifacts contradict one another.
...
- Mocks: `pnpm storybook:host` serves the project selector on `http://127.0.0.1:7456`; `pnpm storybook:mockup` starts the mock Storybook on port 6007; `pnpm build:mockup` builds it to ignored `mockups/dist`. See `docs/storybook-host.md` for container and native hosting.
...
- `mockups/<app>/src/**` uses React, fixtures, local state, and public `@cfreact-template/ui` subpaths. Storybook entries use `@storybook/react-vite` types. Shared `mockups/.storybook`, `mockups/tsconfig.json`, and `mockups/AGENTS.md` define the mock environment, reusing the shared UI Tailwind CSS 4 and React Compiler settings. Production imports formalized app/UI code; mocks remain independent design evidence. Hosting configuration supplies project directories, ports, and public paths at runtime without adding machine-specific settings to projects.
...
- `NONE` has no user-visible surface change and needs no Mock. `CONTINUITY` preserves identified existing production evidence. `SHAPE` uses OpenCode and requires a confirmed Request and owner-accepted Mock before proposal convergence.
...
- OpenCode and the user-selected `openspec/applier` implement only Planning Ready Changes. Their planning-file edits are limited to `tasks.md` progress. UI implementation follows `PRODUCTION_UI -> WIRING -> POLISH -> REVIEW`; unresolved product decisions return to OpenCode.
...
- OpenCode owns `request.md`, `proposal.md`, Specs, `design.md`, and the scope of `tasks.md`. The user selects `openspec/applier` as a primary agent for implementation, never through subagent delegation. Generated skills provide generic traversal within these ownership boundaries.
- Both Change schemas begin with `Request-Status: CONFIRMED` in `request.md`. OpenCode confirms the affected user, current situation, motivation, expected value, and desired outcome as needed. Confirmed Background and Motivation explain the Request but do not create product Requirements by themselves.
- During planning, OpenCode asks one focused question for each material semantic choice not entailed by confirmed content. Clear owner answers are immediately recorded with their confirmation evidence.
