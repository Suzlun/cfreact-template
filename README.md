...
このテンプレートには、AI 支援開発でコード構造が劣化したことを検出するための Sentrux が設定されています。Dev Container と CI は Sentrux の GitHub Releases latest を導入し、`pnpm lint` の中で `sentrux check packages/backend && sentrux check packages/frontend/src` を実行します。OpenCode スキル、ルート設定、運用 scripts、共有UIパッケージは対象外にし、backend/frontend のアプリ本体だけを構造品質ゲートとして監視します。
...
- **`packages/backend/.sentrux/rules.toml`**: backend の循環依存、複雑度、関数行数、主要レイヤー境界の構造ルール
- **`packages/frontend/src/.sentrux/rules.toml`**: frontend app/domain/api の循環依存、複雑度、関数行数、主要レイヤー境界の構造ルール
...
`packages/ui/theme.ts` でテーマをカスタマイズ：
